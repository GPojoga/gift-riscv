...
extern void enc64(unsigned char* input, unsigned char* masterkey, int no_of_rounds, bool print_details) __attribute__((section(".itim")));
...
// key = MSB [31][30]...[1][0] LSB
    ...
    for (int r=0; r<no_of_rounds; r++) // 649409
    {// 23156
        //SubCells
        for (int i=0; i<16; i++) // 530
        {
            input[i] = GIFT_S[input[i]];
        }
        //PermBits
        //input to bits
        for (int i=0; i<16; i++) //2795
        {
            for (int j=0; j<4; j++)
            {
        ...
        //permute the bits
        for (int i=0; i<64; i++) // 1873
        {
            perm_bits[GIFT_P[i]] = bits[i];
        }
        //perm_bits to input
        for (int i=0; i<16; i++) // 3645
        {
            input[i]=0;
            for (int j=0; j<4; j++)
            {
        ...
        //key update
        //entire key>>32
        for(int i=0; i<32; i++) // 914
        {
            temp_key[i] = key[(i+8)%32];
        }

        for(int i=0; i<24; i++) // 565
        {
            key[i] = temp_key[i];
        }
